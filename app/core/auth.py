...
def authorize_user_or_admin(
    permission_name: str,
    get_resource_owner: Callable[
        [int], int
    ],  # function to get owner_id from resource_id
):
    async def dependency(
        ...
        if any(
            role.has_permission(permission_name) for role in current_user.roles
        ):
            return current_user
        ...
        # Check ownership
        owner_id = (
            get_resource_owner(resource_id, session) if resource_id else None
        )
        if owner_id is None:
